...
export class RegisterPersonPage implements OnInit {
    ...
    validarCedula(cedula: string) {
        ...
        if (cedula.length === 10) {
            ...
            if (digitoRegion >= String(1) && digitoRegion <= String(24)) {
                ...
                const pares = Number(cedula.substring(1, 2)) + Number(cedula.substring(3, 4)) + Number(cedula.substring(5, 6)) +
            ...
            console.log('introduzca digitos exactos');
            return false;
        }
    ...
    buscar(event) {
        this.validador = event.detail.value;
    }
}
